...
const string strMessageMagic = "WorldPay Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, uint256 prevHash)
{
	...
	if (nHeight == 1)
        nSubsidy = 8000000 * COIN;	// ICO coins
        
    else if (nHeight <= LAST_POW_BLOCK)
        nSubsidy = (exp(nHeight * 0.0000543776)) * COIN; //Exponential rewards
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "WorldPay", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Trans-Pacific Partnership Trade Deal Is Reached";
        CTransaction txNew;
        txNew.nTime = 1444072573;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1444072573;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
	block.nNonce = !fTestNet ? 250325 : 2056;
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0xe784d03f27da42c202f02a261eacc13f13ba45e7b7dfdcc64375784f913d150c"));
